/*
	6809v.c modified 6809.c to add ACAI and UART as used by Grant Searle
	* 
...
/*
 * VirtualMultiComp for 6809 - Run MultiComp for 6809 
 * Doug Rice May 2020
 *
 * 6809v.c modified 6809.c to add ACAI and UART
 * 
 * 
 ...
 * 
 * 
 * http://www.searle.wales/			multicomp for 6809.
 * In FPGA version UART at 0xFFD0 and 0xFFD1 and uses STDIN and STDOUT
 * In SBC  version UART at 0xA000 and 0xA001 and uses STDIN and STDOUT
 * 
 * Links
 * 
...
/* Added UART based on 68B50 at $FFD0 ,$FFD1 and $A000 ,$A001  */
int uartStatus;
int uartControl;
int uartRx;
int uartTx;
...
static void WRMEM (unsigned addr, unsigned data)
{
  ...
  /* test */	
  /* UART TX control */
  if ( addr == 0xFFD0 || addr == 0xA000  )  
  //if ( addr == 0x9800  )  

  {
	  uartControl = data & 0xFF ;
      uartStatus  = 0x02;
	  //printf ("==IO_W==\n");
	  return;
  }	
...
unsigned RDMEM (unsigned addr)
{
  ...
  //if ( addr == 0x9800  )  
  {
	  ...
	  /* convert LF to CR as 6809 code expects CR */
	  if (	uartRx == 0x0A){ 
		uartRx = 0x0D; 
	  } 
  ...
  if (  addr == 0xFFD2  ) 
  {
    printf ("==IO_R: %04X [%04X]=%02X uart( %04X,%04X,%04X) ==\n", PC, addr, memory[addr], uartStatus, uartTx, uartRx  );
    return uartRx & 0xff ;         
  }	

  if (  addr == 0xFFD3  ) 
  {
    printf ("==IO_R: %04X [%04X]=%02X ==\n", PC, addr, memory[addr]  );
    return uartRx & 0xff ;         
  }	
