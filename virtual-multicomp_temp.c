...
int in(unsigned int port)
{
    ...
    switch ( port) {
      ...
      case 0xB0:
        ...
        if ( uartTxCount == 1 ){
		  /*
		   * On LINUX using stdout seems to buffer char when using BASIC.HEX
		   *
		   * 
		   */
          //printf( "%c",  (char) uartTx );
          //fprintf(stdout, "%c",  (char) uartTx);
          //fprintf(stderr, "%c",  (char) uartTx);
          
          
          //op = fopen( "/dev/pts/0", "a" );
          op = fopen( "/dev/tty", "a" );
          //op = fopen( "/dev/tty2", "a" );
                
          if ( op ){
			  fprintf( op, "%c",  (char) uartTx);
			  fclose ( op );
		  }else{
			  fprintf(stderr, "tty2 not open" );
		  };
